...
# todo: сделать что-то ещё
# в последнее время мне всё труднее и труднее ориентироваться в собственном коде. всё такое цветастое и выделенное...
# мне кажется эти комментарии только мешают пониманию и чтению кода. без них было бы определённо лучше
...
def translate(message):
    ...
    try:                                    # проверяем ввёл ли пользователь текст
        lang = message.text.split(" ", 3)   # делим текст по частям
        first_lang = lang[1]                # язык, с которого нужно перевести. н-р <ru>
        second_lang = lang[2]               # язык, на который нужно перевести. н-р <en>
        text_to_tr = lang[3]                # язык, который нужно перевести
    except IndexError:                      # пишем это ⬇ если пользователь не ввёл что-то из вышеперечисленного
        ...
        return                              # отстанавливаем выполнение команды

    # переходим на страницу с переводчиком с заданными параметрами сверху
    driver.get(f'https://translate.google.com/?hl=ru&sl={first_lang}&tl={second_lang}&op=translate')

    # вводим текст пользователя
    ...
    # тут можно оставить только строчку на две ниже этой. мне просто хочется сделать бота более живым
    try:                                    # проверяем, загрузилась ли страница
        tr_text = driver.find_element_by_xpath("//span[@class='VIiyi']/span/span").text   # сохраняем переведённый текст
    except NoSuchElementException:          # ждём загрузки
        bot.reply_to(message, "Хмм... Мне надо подумать...")    # пользователь думает что так и задумано
        driver.implicitly_wait(10)          # ждём абсурдное количество времени
        tr_text = driver.find_element_by_xpath("//span[@class='VIiyi']/span/span").text   # таки сохраняем текст

    # ждём загрузки транскрипции
    driver.implicitly_wait(1)

    try:                                    # проверяем, загрузилась ли она
        transcript = driver.find_element_by_xpath("//div[@class='UdTY9 BwTYAc Yb6eTe']/div").text   # cохраняем
    except NoSuchElementException:          # а вдруг транскрипции просто нет?
        transcript = "<отсутствует>"        # выводим отсутствие транскрипции

    # господи прости мне 3 строки ниже. более неудобных строчек я ещё не писал
    # добавляем кнопку с заданными пользователем параметрами поиска перевода текста
    ...
    # выводим всё что у нас получилось до этого
    bot.reply_to(message, f'_Минако открывает словарь_\nАх да! Вот перевод твоего слова или фразы: \n"{tr_text}".\nЕго '
                          f'транскрипция: \n"{transcript}"',
                 parse_mode="MARKDOWN", reply_markup=markup)

    # логируем перевод для будущих поколений
...
if __name__ == "__main__":          # проверяем, является ли файл библиотекой
    bot.polling(none_stop=True)     # бот - не библиотека, значит пусть работает как файл
